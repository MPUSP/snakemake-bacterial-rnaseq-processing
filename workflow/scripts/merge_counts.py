...
# -----------------------------------------------------------------------------
#
# This script generats a summary count matrix of all samples
...
try:
    gtf_cols = ['ref', 'source', 'feature_type', 'start', 'stop', 'frame', 'strand', 'score', 'attributes']
    biotypes = pd.read_table(gtf, names=gtf_cols, comment='#')

    bio_type = pd.DataFrame(biotypes.attributes.apply(
                lambda x: x.split(';')).to_list()).loc[:, 2].apply(lambda x: x.lstrip().split(' ')[1].replace('"', ''))

    locus_tag = pd.DataFrame(biotypes.attributes.apply(
                    lambda x: x.split(';')).to_list()).loc[:, 3].apply(lambda x: x.lstrip().split(' ')[1].replace('"', ''))

    name = pd.DataFrame(biotypes.attributes.apply(
                lambda x: x.split(';')).to_list()).loc[:, 4].apply(lambda x: x.lstrip().split(' ')[1].replace('"', ''))
